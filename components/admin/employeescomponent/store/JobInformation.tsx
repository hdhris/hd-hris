  ...
  const jobOptions = jobTitles.reduce((acc: any[], job) => {
    if (job && job.id && job.name && job.is_active) {
      ...
      if (job.is_superior) {
        ...
        // Skip adding the job position if there is already a superior in the department
        // and the current job position is not the one already assigned to an employee in that department
        if (
