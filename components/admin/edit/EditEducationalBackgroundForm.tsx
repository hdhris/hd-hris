...
const EditEducationalBackgroundForm = () => {
    ...
    function updateFileProgress(key: string, progress: FileState["progress"]) {
        setFileStates((prevFileStates) => {
            const newFileStates = prevFileStates.map((fileState) => fileState.key === key ? {
                ...fileState,
                progress
            } : fileState);
    ...
    const handleDownload = async (fileUrl: string, fileName: string) => {
        try {
            ...
            toast({
                title: "Error", description: "Failed to download file. Please try again.", variant: "danger", // Use "danger" variant instead of "destructive"
            });
        }
    };
    ...
    const handleRemove = async (index: number) => {
        try {
            ...
            toast({
                title: "Success", description: "File removed successfully", variant: "success",
            });
        } catch (error) {
            console.error("Error removing file:", error);
            toast({
                title: "Error", description: "Failed to remove file. Please try again.", variant: "danger", // Use "danger" variant instead of "destructive"
            });
        }
    };
    ...
    return (<div className="space-y-6">
            ...
            <FormField
                name="highestDegree"
                control={control}
                render={({field}) => (<FormItem className="col-span-full">
                                        ...
                                        await Promise.all(addedFiles.map(async (addedFileState) => {
                                            try {
                                                ...
                                                const newCertificate: Certificate = {
                                                    fileName: addedFileState.file.name, fileUrl: res.url,
                                                };
