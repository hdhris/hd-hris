...
async function stage_two(prisma: PrismaClient, dateID: number) {
    try {
        ...
        const [cashToDisburse, cashToRepay, benefitsPlansData, payableData] = await Promise.all([
        ...
        return { cashToDisburse, cashToRepay, benefitsPlansData, payableData };
    } catch (error) {
        return NextResponse.json({ error: `Stage one: ${error || "An error occurred"}` }, { status: 500 });
    }
}

async function stage_three(
    prisma: PrismaClient,
    dateID: number,
    calculatedAmountList: Record<number, VariableAmountProp[]>
) {
    try {
        const payheadIDs = Object.entries(calculatedAmountList)
            .flatMap(([_, payheads]) => payheads.map((item) => item.payhead_id))
        ...
        const [breakdowns, ...rest] = await Promise.all([
            prisma.trans_payhead_breakdowns.findMany({
                where: { payroll_id: { in: Array.from(payrollsMap.values()) } },
            }),

            ...Array.from(payrollsMap.values()).map((payrollID) => {
                try {
                    const { earnings, deductions } = salaryAmount[payrollID];
                    return prisma.trans_payrolls.update({
                        where: { id: payrollID },
                        data: {
