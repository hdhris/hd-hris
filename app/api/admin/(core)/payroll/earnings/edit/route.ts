...
export async function POST(
  ...
  try {
    ...
    const payhead = await prisma.ref_payheads.update({
      where: {
        id: id,
      },
      data: {
    ...
    //  const deleteAffectedPromises = affected.map(async (item: number) => {
    //   return prisma.dim_payhead_affecteds.deleteMany({
    //       where: {
    //           payhead_id: id,
    //       }
    //   });
    ...
    //   return prisma.dim_payhead_affecteds.createMany({
    //     data: {
    //       payhead_id: Number(id),
    //       employee_id: item,
    //       created_at: toGMT8(new Date()),
    //       updated_at: toGMT8(new Date()),
    //     },
    ...
    // await Promise.all(deleteAffectedPromises);
    // await Promise.all(createAffectedPromises);

    // Step 1: Fetch existing affected employees for this payhead
    const existingAffected = await prisma.dim_payhead_affecteds.findMany({
      where: { payhead_id: id },
    ...
    if (employeesToDelete.length > 0) {
      await prisma.dim_payhead_affecteds.deleteMany({
        where: {
          payhead_id: id,
          employee_id: {
    ...
    if (employeesToCreate.length > 0) {
      await prisma.dim_payhead_affecteds.createMany({
