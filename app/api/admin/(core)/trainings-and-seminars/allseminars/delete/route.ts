    ...
    try {
        ...
        if (programWithParticipants) {
            return NextResponse.json(
                {
                    status: "error",
                    message: "Cannot delete seminars that has associated participants"
                },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json({
            status: "success",
            message: "seminars marked as deleted",
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
            return NextResponse.json(
                {
                    status: "error",
                    message: "Database error occurred while deleting seminars"
                },
                { status: 500 }
            );
        }
