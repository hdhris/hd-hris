        ...
        try {
            ...
            if (!user) {
                user = await tx.trans_users.create({
                    data: {
                        email: employee.email, name: employee.first_name,
                    },
                });
            }
            ...
            if (data.privilege_id) {
                const existingAccessControl = await tx.acl_user_access_control.findFirst({
                    where: {
                        OR: [{employee_id: id}, {user_id: user.id.toString()}]
                    }
                });
            ...
            if (updates.length > 0) {
                ...
                emailSubject = updates.includes("account_created") ? "Welcome! Your New Account Details" : `Account Update: ${updates.join(" and ")}`;

                try {
                    await sendEmail({
                        to: employee.email, subject: emailSubject, text: emailText,
                    });
                } catch (emailError) {
            ...
            return {
                success: true,
                message: updates.includes("account_created") ? "Account created successfully" : `Account ${updates.join(" and ")} completed successfully`,
                updated: {
            ...
            console.error("Account operation error:", {
                error, stack: error instanceof Error ? error.stack : undefined, timestamp: new Date().toISOString(),
            });
            throw error;
        }
    }, {
