...
const Page: React.FC = () => {
    ...
    SetNavEndContent(() => (<div className="flex items-center gap-4">
    ...
    const handleDelete = async (id: number, name: string) => {
        try {
            const result = await showDialog({
                title: "Confirm Delete", message: `Are you sure you want to delete '${name}' ?`,
            });
            if (result === "yes") {
                await axios.delete(`/api/employeemanagement/branch?id=${id}`);
                toast({
                    title: "Deleted", description: "Branch deleted successfully!", variant: "warning",
                });
                await mutate();
            }
        } catch (error) {
            toast({
                title: "Error", description: "Error: " + error, variant: "danger",
            });
        }
    };
    ...
    const getMunicipalityName = (municipalityId: number | null): string => {
        ...
        const municipality = addressData.find((addr) => addr.address_code === municipalityId);
    ...
    const TableConfigurations = {
        columns: [{uid: "name", name: "Name", sortable: true}, {
            ...
            name: "Municipality",
            sortable: true
        }, {uid: "status", name: "Status", sortable: true}, {uid: "actions", name: "Actions"},],
    ...
    const sortProps = {
        sortItems: [{name: "Name", key: "name" as keyof Branch}, {
            name: "Created At",
    ...
    const FilterItems = [{
        category: "Status",
        filtered: [{key: "is_active", value: true, name: "Active", uid: "active"}, {
            key: "is_active",
            value: false,
            name: "Inactive",
            uid: "inactive"
    ...
    return (<div className="p-4">
            <DataDisplay
                ...
                sortProps={sortProps}
                onListDisplay={(branch) => (<div className="w-full">
            ...
            {selectedBranchId !== null && (<EditBranch
